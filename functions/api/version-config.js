// GET  /api/version-config — return data/version_visibility.json (admins only)
// POST /api/version-config — replace it (admins only). Body: { hidden: ["<base version>", ...] }
//
// Mirrors /api/access: gated on the Cloudflare Access identity in context.data.user,
// commits to GitHub via GITHUB_TOKEN/OWNER/REPO env vars, then Cloudflare Pages rebuilds
// so data/version_visibility.json (read directly by the dashboard) picks up the change.
...
export const onRequestPost = async (context) => {
  ...
  const payload = JSON.stringify({
    _comment: 'Base forecast-version names HIDDEN from the dashboard Compare dropdowns. Empty = all visible. Edit via Admin > Forecast Versions.',
  ...
  try {
    const path = 'data/version_visibility.json';
    const existing = await ghGetFile({ token, owner, repo, branch, path });
    await ghPutFile({ token, owner, repo, branch, path, content: payload,
      sha: existing ? existing.sha : undefined, message: `version-config: updated by ${user.email}` });
    return json({ ok: true });
  } catch (e){
...
async function loadConfig(context){
  try {
    const url = new URL(context.request.url); url.pathname = '/data/version_visibility.json';
    const res = await context.env.ASSETS.fetch(url.toString(), { cf: { cacheTtl: 0 } });
    if (res.ok) return await res.json();
  } catch (e){}
  return { hidden: [] };
