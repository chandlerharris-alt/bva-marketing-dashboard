...
REMAINING (only piece left of the OPEX list): replace the **Source Companies dropdown with an on-grid multi-button selector** above the grid (mirror media's region buttons in `buildMediaGrid`), region tabs showing only that region's companies. Deferred to keep the grid/filterbar change verifiable on its own; the dropdown still works meanwhile.
...
## Git: latest commit `597f95e` (OPEX PY/Plan/Actuals grid + simplified filterbar). Push after each change; Cloudflare auto-rebuilds ~1-2 min.
