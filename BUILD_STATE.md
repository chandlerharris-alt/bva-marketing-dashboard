...
## Git: latest commit `5727986` (version limiting + drill redesign). Push after each change; Cloudflare auto-rebuilds ~1-2 min. NOTE: `git pull` before pushing — Cloudflare Functions commit admin saves to main, so origin often moves ahead. Still open: Source Companies dropdown → on-grid buttons (OPEX).
